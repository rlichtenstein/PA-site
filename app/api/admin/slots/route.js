...
export async function GET() {
  const { rows } = await query(
    `select
       ...
       s.start_time, s.end_time, s.location, s.capacity, s.created_at,
...
export async function POST(request) {
  ...
  const { title, category, description, event_date, start_time, end_time, location, capacity } =
    body || {};

  if (!title || !event_date || !capacity) {
    return NextResponse.json(
      { error: 'title, event_date, and capacity are required.' },
      { status: 400 }
    );
  }
  ...
  const { rows } = await query(
    `insert into slots (title, category, description, event_date, start_time, end_time, location, capacity)
     values ($1, $2, $3, $4, $5, $6, $7, $8)
     returning id, title, category, description, event_date::text as event_date,
               start_time, end_time, location, capacity, created_at`,
    [
      title,
      category || 'General',
      description || null,
