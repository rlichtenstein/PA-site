...
async function sendSignupConfirmation({ slot, signup }) {
  ...
  const calendarUrl = `${siteUrl()}/api/signups/${signup.cancel_token}/calendar`;
...
async function sendOrganizerSignupNotification({ slot, signup }) {
  if (!slot.contact_email) return null;
  const when = formatSlotWhen(slot);
  const text = `${signup.parent_name} has signed up for ${slot.title} on ${when}.`;

  return sendEmail({
    to: slot.contact_email,
    subject: `New volunteer for ${slot.title}`,
    text,
  });
}
...
module.exports = {
  ...
  sendSignupReminder,
  sendOrganizerSignupNotification,
  formatSlotWhen,
};
